...
int Init_SDL(int edge, int frame_width, int frame_height){
	...
	// yuv params
	yuv[0] = (unsigned char *)malloc((frame_width + 2 * edge) * frame_height * sizeof(unsigned char));
	yuv[1] = (unsigned char *)malloc((frame_width + edge) * frame_height / 4 * sizeof(unsigned char));
	yuv[2] = (unsigned char *)malloc((frame_width + edge) * frame_height / 4 * sizeof(unsigned char));
...
void SDL_Display(int edge, int frame_width, int frame_height, unsigned char *Y, unsigned char *U, unsigned char *V){
	...
	if (frame_width != screen -> w || frame_height != screen -> h){
		...
		screen = SDL_SetVideoMode(frame_width, frame_height, 0, SDL_SWSURFACE);
