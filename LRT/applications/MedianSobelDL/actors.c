...
//	printf("in %#x out %#x nbSLices %#x width %#x height %#x\n", in, out, &nbSlices, &width, &height);
//	printf("i %#x j %#x g %#x k %#x\n", &i, &j, &g, &k);
...
//			// Histo
//			if(j-2>=0){
//				histo[in[(i-1)*width+(j-2)]]--;
//				histo[in[(i)  *width+(j-2)]]--;
//				histo[in[(i+1)*width+(j-2)]]--;
//			}
//
//			histo[in[(i-1)*width+(j+1)]]++;
//			histo[in[(i  )*width+(j+1)]]++;
//			histo[in[(i+1)*width+(j+1)]]++;
//
//			int median,n;
	...
	for(i=1; i<height/nbSlices-1; i++){
		...
		for(j=1; j<width-1; j++){
			...
			for(pi=0; pi<5; pi++){
				minIx = 0;
				for(pj=1; pj<9-1; pj++){
					if(valid[pj] && array[pj] < array[minIx]){
						minIx = pj;
					}
				}
				valid[minIx]=0;
