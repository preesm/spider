...
for i in $(seq 0 $NB_DSP) ; do
	echo -ne "\nLoading on DSP$i... "
	OUTPUT=$(ssh $SSH_REMOTE_USER@$SSH_REMOTE_HOST "mpmcl reset dsp$i")
	check_error $? "$OUTPUT"

	OUTPUT=$(ssh $SSH_REMOTE_USER@$SSH_REMOTE_HOST "mpmcl load dsp$i $REMOTE_WORKING_PATH/$DSP_REMOTE_BIN")
        check_error $? "$OUTPUT"

	OUTPUT=$(ssh $SSH_REMOTE_USER@$SSH_REMOTE_HOST "mpmcl run dsp$i")
        check_error $? "$OUTPUT"
