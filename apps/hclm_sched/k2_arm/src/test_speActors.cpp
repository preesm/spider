...
void testSpeActor(Stack* pisdfStack, Archi* archi){
	ExecutionStat stat;
	FILE* fres;

	try{
#define SH_MEM 0x00500000
		spider_setMemAllocType(MEMALLOC_DUMMY, 0, SH_MEM);
		spider_setGraphOptim(true);
		spider_setActorPrecedence(true);

		fres = fopen("speActorNotN.csv","w+");
