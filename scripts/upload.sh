...
function checkmd5 {
  ...
  REMOTEMD5=$(ssh $SSH_OPTIONS $SSH_REMOTE_USER@$SSH_REMOTE_HOST -- md5sum $REMOTEFILE | cut -d' ' -f1)
...
ssh $SSH_OPTIONS $SSH_REMOTE_USER@$SSH_REMOTE_HOST "mkdir -p $REMOTE_WORKING_PATH"
...
if [ $? -eq 0 ]; then
  scp $SCP_OPTIONS $SPIDER_K2ARMLIB_OUTPATH/$CURRENT_ARMLIB_FILENAME $SSH_REMOTE_USER@$SSH_REMOTE_HOST:/usr/lib/$CURRENT_ARMLIB_FILENAME
fi

#Uploading the ARM binary
checkmd5 "$CURRENT_ARM_BIN_PATH" "$REMOTE_WORKING_PATH/$(basename $CURRENT_ARM_BIN_PATH)"
if [ $? -eq 0 ]; then
  scp $SCP_OPTIONS $CURRENT_ARM_BIN_PATH $SSH_REMOTE_USER@$SSH_REMOTE_HOST:$REMOTE_WORKING_PATH
fi

#Uploading the DSP binary
checkmd5 "$CURRENT_DSP_BIN_PATH" "$REMOTE_WORKING_PATH/$(basename $CURRENT_DSP_BIN_PATH)"
if [ $? -eq 0 ]; then
  scp $SCP_OPTIONS $CURRENT_DSP_BIN_PATH $SSH_REMOTE_USER@$SSH_REMOTE_HOST:$REMOTE_WORKING_PATH
fi
